...
class TemporalDetectionTracker:
    ...
    def __init__(
        ...
        *,
        ...
        reinit_after_incompatible_frames: int,
        min_iou: float = 0.10,
    ) -> None:
        ...
        self.reinit_after_incompatible_frames = max(1, int(reinit_after_incompatible_frames))
    ...
    def update(self, candidate: "DetectionCandidate" | None) -> TrackedDetection | None:
        ...
        if not self.is_compatible(self._track, candidate):
            return self.handle_incompatible_detection(candidate)
    ...
    def handle_incompatible_detection(self, candidate: "DetectionCandidate") -> TrackedDetection | None:
        if self._track is None:
            self._track = self.create_track(candidate)
            return self._track if self._track.confirmed_frames >= self.min_confirmed_frames else None

        self._track.incompatible_frames += 1
        if self._track.incompatible_frames < self.reinit_after_incompatible_frames:
            self._track.stale = True
            self._track.missed_frames = min(self._track.missed_frames + 1, self.max_missed_frames)
            self._track.confidence *= 0.97
            return self._track if self._track.confirmed_frames >= self.min_confirmed_frames else None
    ...
    def create_track(self, candidate: "DetectionCandidate") -> TrackedDetection:
        track = TrackedDetection(
            ...
            left_xyz=candidate.left_xyz,
            right_xyz=candidate.right_xyz,
    ...
    def update_track(self, track: TrackedDetection, candidate: "DetectionCandidate") -> TrackedDetection:
        ...
        if track.left_xyz is None:
            track.left_xyz = candidate.left_xyz
        elif candidate.left_xyz is not None:
            track.left_xyz = tuple(
                self.smooth_scalar(old, new, self.center_smoothing_alpha)
                for old, new in zip(track.left_xyz, candidate.left_xyz)
            )

        if track.right_xyz is None:
            track.right_xyz = candidate.right_xyz
        elif candidate.right_xyz is not None:
            track.right_xyz = tuple(
                self.smooth_scalar(old, new, self.center_smoothing_alpha)
                for old, new in zip(track.right_xyz, candidate.right_xyz)
            )
