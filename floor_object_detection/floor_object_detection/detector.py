...
class DepthBasedFloorObjectDetector:
    ...
    def detect(
        ...
        *,
        ...
        edge_candidate, edge_mask = self.detect_with_edge_fallback(
            color_bgr=color_bgr,
            depth_m=depth_m,
        ...
        if edge_candidate is not None:
            self._last_edge_bbox = edge_candidate.bbox
            return DetectionFrame(
                candidate=edge_candidate,
                foreground_mask=edge_mask,
                floor_mask=empty_mask,
                edge_mask=edge_mask,
                plane=None,
                used_fallback=True,
                status_text="edge-assisted primary",
            )
        ...
        if candidate is None:
            ...
            if candidate is not None:
                status_text = f"{status_text} | relaxed"
    ...
    def find_best_candidate(
        ...
        *,
        ...
        best_candidate = None
        best_score = -math.inf
        thresholds = self.get_candidate_thresholds(relaxed=relaxed)

        for contour in contours:
            area = float(cv2.contourArea(contour))
            if area < thresholds["min_contour_area"]:
                continue

            x, y, w, h = cv2.boundingRect(contour)
            if w < thresholds["min_bbox_size_px"] or h < thresholds["min_bbox_size_px"]:
                continue

            aspect = w / float(max(h, 1))
            max_bbox_aspect_ratio = thresholds["max_bbox_aspect_ratio"]
            if aspect > max_bbox_aspect_ratio or aspect < (1.0 / max_bbox_aspect_ratio):
                continue
            ...
            if support_pixels < thresholds["min_support_pixels"]:
                continue
